...
def update
  ...
  params_fields[:name].each do |field_name|
    @fields.merge! field_name => params_fields[:text][i] if !field_name.empty?
    i += 1
  end
  @user = User.find(params[:id])
  if @user.update_attributes(user_params.merge fields: JSON.dump(@fields))
